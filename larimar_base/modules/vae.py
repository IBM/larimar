...
class VAE(nn.Module):
    ...
    def ae(self, x0, x1, read_write=False, sample=False, read_iters=1):
        ...
        mean, logvar = self.encoder.linear(bert_fea).chunk(2, -1)
        if sample:
            z = self.reparameterize(mean, logvar, nsamples=1)
        else:
            z = mean
        outputs = self.decoder(input_ids=x1, past=z, labels=x1, label_ignore=self.pad_token_id)
        return outputs[0], z
    ...
    def loss_iw(self, x0, x1, nsamples=50, ns=1):
        ...
        log_gen_z = self.eval_cond_ll(x1, mu)
        ...
        return log_prob_iw, log_gen_iw , KL, log_gen_z
