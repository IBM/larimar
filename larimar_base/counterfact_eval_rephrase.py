...
# from utils_mem import write, read
import random
random.seed(42)
...
class MENDQADataset:
    ...
    def __init__(self, data_dir: str, tok: AutoTokenizer, size=None, idx_start=None): #ELN
...
class CounterFactDataset(Dataset):
    def __init__(self, data_dir: str, size: typing.Optional[int] = None, idx_start: typing.Optional[int] = None, *args, **kwargs):
        data_dir = Path(data_dir)
        ...
        if idx_start is None: #ELN
            self.data = self.data[idx_start:]
        if size is not None:
...
def sample_sequence_conditional(model,
    ...
    with torch.no_grad():
        while True:
            ...
            if tok_new and tok_true:
                probs = torch.nn.functional.log_softmax(outputs[0][0, -1, :])
                gen_hits.append((probs[tok_new] > probs[tok_true]).item())
    ...
    if gen_hits:
        return generated, sum(gen_hits)/len(gen_hits)
...
def compute_rewrite_quality_zsre(model, tokenizer, record, posterior_memory, scope_detector=None, no_memory=False, gc_mem=None):
    ...
    stuff_probs = test_batch_prediction_acc(model, tokenizer, inp_prompts, inp_targets_tok,
                                            posterior_memory, scope_detector, no_memory=no_memory, gc_mem=gc_mem)
...
def test_batch_prediction_acc(model, tok, prompts, target, posterior_memory, scope_detector=None, no_memory=False, gc_mem=None):

    enc_tok, dec_tok = tok

    # prompts_tmp = [prompts[0]]*len(prompts)
    encoded_read, cls = latent_code_from_text(prompts, enc_tok, model, device='cuda')
    if gc_mem:
        zs = read(model, enc_tok, encoded_read)
    else:
...
#            zs, _ = model.read(encoded_read.unsqueeze(1), posterior_memory, deterministic=True) ## ELN - removing sigma_w arg
            zs = zs.squeeze()
    ...
    if paraphrase_prompts:
        ...
        for para in paraphrase_prompts:

            if scope_detector is not None:
                if scope_detector.is_indomain(para)[0]:
                    past = latent_z
                else:
                    past = torch.zeros_like(latent_z)

            past = latent_z

            context_tokens = tok.encode('<BOS>' + para)
            context = torch.tensor(context_tokens, dtype=torch.long, device=torch.device('cuda'))

            out, hits = sample_sequence_conditional(
                model=model.decoder,
            ...
            eos_token_val = tok.encode('<EOS>')[0]
            tokens = out[0, 1:].tolist()  # filter out BOS
            if eos_token_val in tokens:
                loc = tokens.index(eos_token_val)
                tokens = tokens[:loc]  # end before EOS
            text = tok.decode(tokens, clean_up_tokenization_spaces=True)
            text = text.split()  # [1:-1]
            text = ' '.join(text)
            pure_gen = text[len(para):]
            if target_new.lower() in pure_gen.lower():
                gen_para.append({pure_gen: True})
            else:
                gen_para.append({pure_gen: False})
            gen_para.append({pure_gen: hits})
    # -------------------------------------------
...
def eval(model, tokenizer, record, dataset, snips, vec, mode='pyrite', deterministic=True,
         scope_detection_threshold=None, counterfact_plus=None, sigma_w=None, gc_mem=None, 
         num_rephrases=0, multiplier=1., use_keys_from_prompts=False, 
         use_ben_cohen_approx=False, seen_distractor=False, remove_distraction=False):
    ...
    if remove_distraction:
        record['paraphrase_prompts'] = [" ".join(sent_tokenize(x)[1:]) for x in record['paraphrase_prompts']]

    if num_rephrases==1:
        half = len(record['paraphrase_prompts']) // 2
        add_to_memory = record['paraphrase_prompts'][:half]
        record['paraphrase_prompts'] = record['paraphrase_prompts'][half:]  # use second half to query
        if len(add_to_memory)>1:
            print("WARNING: Len of memory to add is ", len(add_to_memory))

        for r in add_to_memory:
            text_write_to_mem.append(r + " " + record["requested_rewrite"]['target_new']['str'])
    elif num_rephrases>1:
        half = len(record['paraphrase_prompts']) // 2
        add_to_memory = record['paraphrase_prompts'][:half]
        record['paraphrase_prompts'] = record['paraphrase_prompts'][half:]
        already_added = len(add_to_memory)
        if already_added>1:
            print("WARNING: Len of memory to add is ", len(add_to_memory))
        ...
        all_distractors=[]
        for r in add_to_memory:
            sent=r + " " + record["requested_rewrite"]['target_new']['str']
            text_write_to_mem.append(sent)
            if not remove_distraction:
                distractor=sent_tokenize(sent)[0]
                all_distractors.append(distractor)
        
        if seen_distractor and not remove_distraction:
            for r in record['paraphrase_prompts']:
                sent=r + " " + record["requested_rewrite"]['target_new']['str']
                distractor=sent_tokenize(sent)[0]
                all_distractors.append(distractor)
        ...
        try:
            if num_rephrases>0:
                synth_rewrites=record['mixtral_rewrite']
        except Exception as e:
            print(e)
            synth_rewrites=[]

        if num_rephrases>0 and len(synth_rewrites)>0:
            simple_sentences = random.sample(synth_rewrites, num_rephrases-1)
            if remove_distraction:
                text_write_to_mem += simple_sentences
            else:
                text_write_to_mem += [random.choice(all_distractors)+" "+x for x in simple_sentences]
        
    # print("Total number of elements in memory:", len(text_write_to_mem))
    prompts_batch = text_to_query
    # print('text_to_query = ', text_to_query)
    # print('NOT adding anything else to memory')

    #### ELN
    keys_from_prompts = use_keys_from_prompts # if True, compute writing weights from encodings of prompts
    model.memory.ben_cohen_approx = use_ben_cohen_approx # if False, use torch.linalg.pinv() to compute pseudoinverses
    # print('keys_from_prompts = ', keys_from_prompts)
    # print('model.memory.ben_cohen_approx = ', model.memory.ben_cohen_approx)
    ...
    if mode == 'pyrite' or mode == 'combo':
        ...
        if keys_from_prompts:
            prompts_encoded, _ = latent_code_from_text(prompts_batch, enc_tok, model, device='cuda')
            prompts_encoded = prompts_encoded.reshape(len(prompts_batch), 1, model._code_size)
            posterior_memory, dkl_M = model.write(input_encoded=memory_write_text_encoded, input_encoded_key=prompts_encoded)
        elif gc_mem: ## this needs to be improved -Elliot
            posterior_memory = None
            _ = write(model, tokenizer, memory_write_text_encoded)
        else:
            posterior_memory, dkl_M = model.write(input_encoded=memory_write_text_encoded)
        ...
        if dataset == 'counterfact':
            ...
            output = compute_rewrite_quality_zsre(model, tokenizer, record, posterior_memory,
                                                  scope_detector=scope_detector,
                                                  gc_mem=gc_mem)
...
def main(args):
    ...
    ## ELN
    model.memory.use_ref_mem = False # if True, use a fixed reference memory matrix to compute reading and writing weights
    random_init_mem = False # if True, set M_0 to a random Gaussian matrix
    print('model.memory.use_ref_mem = ', model.memory.use_ref_mem)
    print('random_init_mem = ', random_init_mem)
    if random_init_mem:
        mem_random = np.random.normal(scale=1, size=(model._memory_size, model._code_size))
        model.memory.memory_mean = torch.nn.Parameter(torch.from_numpy(mem_random).float().to('cuda'))
    if model.memory.use_ref_mem:
        model.memory.memory_ref = model.memory.memory_mean.clone()
    ...
    if args.dataset:
        ...
        if args.dataset == 'zsre':
            ...
            enc_tok, dec_tok = tokenizer
            ds = MENDQADataset(args.data_dir, tok=dec_tok, size=args.num_eval_cases, idx_start=args.idx_start) #ELN
            snips, vec = None, None
        else:  # counterfact
            print("Loading dataset, attribute snippets, tf-idf data")
            snips = AttributeSnippets(args.data_dir)
            vec = get_tfidf_vectorizer(args.data_dir)
            ds = CounterFactDataset(args.data_dir, size=args.num_eval_cases, idx_start=args.idx_start) #ELN
            snips = None

        print("### Start idx: ", args.idx_start)
        print("Total number of elements in dataset is:", len(ds))
        ...
        for record in tqdm(ds):
            ...
            if not case_result_path.exists():
                out = eval(model=model,
                           ...
                           use_keys_from_prompts=args.use_keys_from_prompts,
                           use_ben_cohen_approx=args.use_ben_cohen_approx,
                           seen_distractor=args.seen_distractor,
                           remove_distraction=args.remove_distraction)
        ...
        if args.dataset == 'counterfact':
            ...
            final_result = {'rewrite/efficacy': results[0]['post_rewrite_acc'][0],
                            ...
                            'model': args.checkpoint}
    if args.ppl2:
        assert not model.memory.use_ref_mem ## (ELN) else we should generalize model.write() in gen_ppl() below, as in eval() above; also gen_ppl() calls read() with sigma_w=sigma_w
        device = "cuda"
        model_id = "gpt2-large"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--idx_start", type=int, default=0, help="for evaluating with a block of data starting at idx_start, rather than the whole dataset") #ELN
    parser.add_argument("--gc_mem", action="store_true", help="GPM vs Gaussian Convolution Memory")
    parser.add_argument("--num_rephrases", type=int, default=0)
    parser.add_argument("--multiplier", type=float, default=1.)
    parser.add_argument("--use_keys_from_prompts", action="store_true", help="use_keys_from_prompts")
    parser.add_argument("--use_ben_cohen_approx", action="store_true", help="use_ben_cohen_approx")
    parser.add_argument("--seen_distractor", action="store_true", help="Use seen distractor")
    parser.add_argument("--remove_distraction", action="store_true", help="Use seen distractor")
